    ...
    const handleSubmit = async (event) => {
        ...
        ['firstName', 'lastName', 'email', 'message'].forEach((field) => {
            if (!formData.get(field)) {
                newErrors[field] = 'Bu alan gereklidir';
            }
        });

        if (!agreed) {
            newErrors.agreed = 'Gizlilik politikasını kabul etmelisiniz';
        }
            ...
            <section id='contacttr'>
                <div className="mx-auto max-w-2xl text-center">
                    ...
                    <p className="mt-2 text-lg text-[rgb(245,215,150,0.8)] mb-8">
                        Herhangi bir sorunuz veya destek ihtiyacınız için ekibimizle iletişime geçin.
                    </p>
                </div>
                <form onSubmit={handleSubmit} className="mx-auto mt-10 max-w-xl sm:mt-14">
                    <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
                        {[{ label: 'Ad', name: 'firstName' }, { label: 'Soyad', name: 'lastName' }, { label: 'E-posta', name: 'email' }].map(({ label, name }, index) => (
                        ...
                        <div className="sm:col-span-2">
                            <label className="block text-sm font-medium text-[rgb(245,215,150)]">Mesaj</label>
                            <textarea
                                rows={4}
                                name="message"
                        ...
                        <div className="sm:col-span-2 flex items-center gap-x-4">
                            ...
                            <label className="text-sm text-[rgb(245,215,150)]">Gizlilik politikasını kabul etmelisiniz.</label>
                        </div>
                    ...
                    <div className="mt-10">
                        <button type="submit" disabled={loading} className="w-full rounded-lg bg-purple-700 px-5 py-3 text-white text-lg font-semibold shadow-md cursor-pointer">{loading ? 'Gönderiliyor...' : 'Gönder'}</button>
                    </div>
                </form>
            </section>
        </div>
